...
$(document).on("click", "#btnEditar", function() {
	...
	const inputNombre = crearElemento('input', 'text', 'nombre', '', 'Nombre..');
			...
			.then(function(response) {
				if (response.ok) {
					...
					throw "La oferta ya existe";
						...
						var pais = tr.childNodes[5];
						pais.innerText = comida.pais;

		/*		let tbody = document.getElementById("root");
				for (var i = 0, root; root = tbody.childNodes[i]; i++) {
					for (var j = 0, div; div = root.childNodes; j++) {
						var arrayElements = div;
						if (arrayElements[3].childNodes[1].innerText == comida.id_comida ) {
							arrayElements[3].childNodes[5].innerText = comida.pais;
						}
						break;
					}
				}*/
...
//ANADIR AL INDEX AL CREARLA
const addComidaIndex = (comidas) => {
	resultado += `<div class="card">
							<img alt="IMAGEN"></img>
							<div class="card-body">
								<h5 class="card-title" id='${comidas.id_comida}'>${comidas.id_comida}</h5>
								<h5 class="card-title">${comidas.nombre}</h5>		
								<h5 class="card-title">${comidas.pais}</h5>		
								<p class="card-text">${comidas.descripcion}</p>
								<button class="btn btn-danger" id="btnBorrar">Borrar</button>
								<button class="btn btn-info" id="btnEditar">Editar</button>
							</div>
						   </div>`
	root.innerHTML = resultado;
}

//CARGAR TODAS LAS COMIDAS AL INDEX
const mostrarTodas = (comidas) => {
	comidas.forEach(comida => {
		resultado += `<div class="card">
                            <img alt="imagen"></img>
                                <div class="card-body">
                                	<h5 class="card-title" id='${comida.id_comida}'>${comida.id_comida}</h5>
                                    <h5 class="card-title">${comida.nombre}</h5>
                                    <h5 class="card-title">${comida.pais}</h5>		
